...
public class Syllabus extends Submenu {
    ...
    public void fillContentView(final Context context) {
        Student.getInstance().getRequestMaker().makeGetRequest("https://ogr.kocaeli.edu.tr/KOUBS/Ogrenci/DersIslemleri/DersProgramiIcerik.cfm", new ConnectionListener() {
            @Override
            public void onSuccess(String... args) {
                ...
                for (Day day : days) {
                    if (day.classes.size() == 0) continue;
...
class Day implements Serializable {
        ...
        for (int i = 0; i < classes.size(); i++) {
            ...
            if (i + 1 < classes.size()) {
                if (classes.get(i).endTime.getTime() < classes.get(i + 1).startTime.getTime()) {
                    ...
                    String text = (classes.get(i + 1).startTime.getTime() - classes.get(i).endTime.getTime()) + " " + "dakika ara";
    ...
    public void addClass(Class toAdd) {
        for (int i = 0; i < this.classes.size(); i++) {
            ...
            if (!cl.name.equals(toAdd.name)) continue;
